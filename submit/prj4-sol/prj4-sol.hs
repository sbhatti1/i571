...
categoryItems [] _ = []  -- base case: empty list
...
normalizeBindings bindings = 
  let sortedBindings = sortOn fst bindings
      normalizedBindings = map (\(varName, term) -> (varName, substTerm term sortedBindings)) sortedBindings
